...
2.  **Installation of Application Configurations**:
    *   The configuration folders for applications (`foot`, `nvim`, `sway`, `swaylock`, `waybar`, `wofi`, `mako`, `fastfetch`, `hypr`, `ghostty`, `rmpc`, `mpd`, `yazi`, `geany`) are copied from the repository directory to `$HOME/.config/`.
